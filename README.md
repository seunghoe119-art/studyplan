...
- 순수 정적 사이트 (React + Babel을 CDN에서 불러와 브라우저에서 바로 실행, 빌드 과정/백엔드 없음)
- 파일 구조: `index.html` + `app/` 폴더 (Dashboard/Timer/Progress/Numbers/Settings 화면, schedule.js 일정 엔진, styles.css)
...
`repo` 폴더(= `index.html`과 `app` 폴더가 같은 위치에 있는 상태) 전체를 유지한 채로 `index.html`을 더블클릭해서 브라우저로 열면 인터넷 연결만 있으면(React/폰트를 CDN에서 불러옴) 동일하게 동작합니다. `index.html`만 따로 빼서 열면 `app/` 폴더를 못 찾아 화면이 뜨지 않으니 주의하세요.

## 나중에 수정하고 싶을 때

`index.html`과 `app/` 폴더 전체가 배포 대상입니다. 필요한 부분을 고치고
